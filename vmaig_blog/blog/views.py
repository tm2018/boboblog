...
class CategoryView(BaseMixin, ListView):
    ...
    def get_queryset(self):
        ...
        category_list = self.get_child_category(category)
        #如果category_list的值为none，则表明子类为空，否则要把类和子类加入list
        ...
        for category in category_list:
            ...
            try:
                #用name去查询数据库的title，然后把查询到的queryset加入到chain中
                article_list = Category.objects.get(name=category).article_set.all()
                article_all_lists = chain(article_all_lists,article_list)
            except Category.DoesNotExist:
                logger.error(u'[CategoryView]此分类不存在:[%s]' % category)
                raise Http404
        #将chain取出来组成一个list后return
        for i in article_all_lists:
            article_lists.append(i)
        return article_lists
    #判断是否是父类，如果是父类则还要查找到子类
    def get_child_category(self,category):
        from django.db import models
        #查询数据库取出所有分类的name,id及parent_id
        all_category = Category.objects.all().values_list('name','id','parent_id')
        for i in all_category:
            l = list(i)
            if l[0] == category:
                my_id = l[1]
        #如果发现有文章的父id是当前分类的id，则获取其所有文章
        child_name = [x[0] for x in all_category if x[2]==my_id]
        return child_name
